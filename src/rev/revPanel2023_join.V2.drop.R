...
# Archivo: src/rev/revPanel2023_join.V2.drop.R
#
# Variante de sensibilidad: elimina del panel los registros incluidos en
# riesgo_total antes de calcular los totales y medias del alquiler.
#
...
riesgo_mixto <- dt[INGRESOS_REALES > 0 & !is.na(RC_ANONIMA) & VIV > 1 & (is.na(URBACLAVES_HABITUAL) | !grepl("V", URBACLAVES_HABITUAL)) & 
...
dt[, es_riesgo_total := !is.na(INGRESOS_REALES) & INGRESOS_REALES > 0 &
     (is.na(URBACLAVES_HABITUAL) | !grepl("V", URBACLAVES_HABITUAL)) &
     (is.na(REDUCCION_REAL) | REDUCCION_REAL <= 0) &
     !is.na(VIV) & VIV >= 1]
riesgo_total <- dt[es_riesgo_total == TRUE,
                   .(n = .N, n_mixto = sum(VIVLOC > VIV, na.rm = TRUE))]
...
# Sensibilidad: excluir solo los registros con la senal de riesgo observada
# (VIVLOC > VIV). El total de riesgo_total incluye tambien registros validos
# para el dominio de alquiler no habitual residencial.
dt[, es_riesgo_drop := es_riesgo_total & !is.na(VIVLOC) & VIVLOC > VIV]
n_riesgo_drop <- dt[es_riesgo_drop == TRUE, .N]
dt <- dt[es_riesgo_drop != TRUE]
cat(sprintf("Registros excluidos por riesgo_total (VIVLOC > VIV):          %s\n\n",
            fmt_num(n_riesgo_drop)))
